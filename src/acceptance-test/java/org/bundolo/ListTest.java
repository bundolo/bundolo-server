...
public class ListTest extends TestBase {
	...
	rows.get(26).click();
	Thread.sleep(500);
	Assert.assertEquals("Text page title not as expected.", "tekst - " + textTitle + " - bundolo", getDriver()
		.getTitle());
    }
	...
	rows.get(34).click();
	Thread.sleep(500);
	Assert.assertEquals("Serial page title not as expected.", "serija - " + serialTitle + " - bundolo", getDriver()
		.getTitle());
    }
	...
	rows.get(43).click();
	Thread.sleep(500);
	Assert.assertEquals("Author page title not as expected.", "autor - " + authorTitle + " - bundolo", getDriver()
		.getTitle());
    }
	...
	((JavascriptExecutor) getDriver()).executeScript("$('#" + TestConstants.ANNOUNCEMENTS
		+ ">tbody').scrollTop(300)");
	Thread.sleep(500);
	...
	// ((JavascriptExecutor) getDriver()).executeScript("$('#" + TestConstants.CONTESTS +
	// ">tbody').scrollTop(300)");
	// Thread.sleep(500);
	// rows = getDriver().findElement(By.id(TestConstants.CONTESTS)).findElement(By.tagName("tbody"))
	// .findElements(By.tagName("tr"));
	// Assert.assertEquals("Wrong number of contests after scrolling.", 35, rows.size());
