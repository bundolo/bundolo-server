...
public class ContentServiceImpl implements ContentService {
	...
	public Content getPageDescriptionContent(PageKindType pageKind) {
		...
		if (pageDescriptionContent != null) {
			...
			Rating rating = pageDescriptionContent.getRating().size() > 0
					? (Rating) pageDescriptionContent.getRating().toArray()[0] : null;
			if (rating == null) {
	...
	public Content findAnnouncement(String slug) {
		// TODO replacing all find methods by single findBySlug which would
		// handle all
		...
		if (announcement != null && ContentKindType.news.equals(announcement.getKind())) {
			...
			Date lastActivity = !announcement.getAuthorUsername().equals(SecurityUtils.getUsername()) || rating == null
	...
	public Content findSerial(String slug) {
		...
		if (serial != null) {
			...
			long ratingIncrement = serial.getAuthorUsername().equals(SecurityUtils.getUsername()) ? 0
					: Constants.DEFAULT_RATING_INCREMENT;
			Date lastActivity = !serial.getAuthorUsername().equals(SecurityUtils.getUsername()) || rating == null
					? dateUtils.newDate() : rating.getLastActivity();
			if (rating == null) {
				rating = new Rating(null, null, RatingKindType.general, lastActivity, RatingStatusType.active,
	...
	public Content findText(String slug) {
		...
		if (text != null) {
			...
			long ratingIncrement = text.getAuthorUsername().equals(SecurityUtils.getUsername()) ? 0
					: Constants.DEFAULT_RATING_INCREMENT;
			Date lastActivity = !text.getAuthorUsername().equals(SecurityUtils.getUsername()) || rating == null
					? dateUtils.newDate() : rating.getLastActivity();
			if (rating == null) {
				rating = new Rating(null, null, RatingKindType.general, lastActivity, RatingStatusType.active,
	...
	public Content findTopic(String slug) {
		...
		if (topic != null) {
			...
			Date lastActivity = topic.getAuthorUsername() == null
					|| !topic.getAuthorUsername().equals(SecurityUtils.getUsername()) || rating == null
							? dateUtils.newDate() : rating.getLastActivity();
			if (rating == null) {
				rating = new Rating(null, null, RatingKindType.general, lastActivity, RatingStatusType.active,
		...
		try {
			...
			if (ContentKindType.episode.equals(content.getKind())) {
				if (content.getParentContent().getContentId() == null) {
					...
					return new ResponseEntity<String>(ReturnMessageType.episode_detached.name(),
				...
				if (ContentStatusType.active.equals(parentContent.getContentStatus())) {
					// if serial is active, adding new episode is not allowed if
					// last one is pending
		...
		try {
			...
			if (senderUsername != null || anonymousAllowed) {
				if (content.getContentId() == null) {
					...
					if ((ContentKindType.episode.equals(content.getKind())
							|| ContentKindType.episode_group.equals(content.getKind()))
							&& content.getContentStatus() != null) {
	...
	public Content findEpisode(String slug) {
		...
		if (episode != null) {
			...
			long ratingIncrement = episode.getAuthorUsername().equals(senderUsername) ? 0
					: Constants.DEFAULT_RATING_INCREMENT;
			Date lastActivity = !episode.getAuthorUsername().equals(senderUsername) || rating == null
					? dateUtils.newDate() : rating.getLastActivity();
			if (rating == null) {
				rating = new Rating(null, null, RatingKindType.general, lastActivity, RatingStatusType.active,
	...
	public Long deleteEpisode(String slug) {
		...
		if (episode == null) {
			...
			if (ContentStatusType.active.equals(episode.getParentContent().getContentStatus())) {
				// don't allow deleting episode in active serial if it's not
				// last
				Content nextEpisode = contentDAO.findNext(episode.getContentId(),
						TextColumnType.valueOf("date").getColumnName(), null, true);
				if (nextEpisode != null) {
