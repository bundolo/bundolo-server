...
public class RestAuthenticationProvider implements AuthenticationProvider {
	...
	if (StringUtils.isNotBlank(key) && StringUtils.isNotBlank(credentials)) {
	    ...
	    if (userProfile != null) {
		try {
		    if (SecurityUtils.getHashWithPredefinedSalt(credentials, userProfile.getSalt()).equals(
    ...
    // this version is safer because each request is checked, but there is extra query in case of anonymous access
    /*    
        @Override
        public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    	// logger.log(Level.WARNING, "authenticate authentication: " + authentication);
